	...
	for (iter = 0; iter < 50; iter++) {
		...
		for (i = 0; i < 1000; i++) {
			...
			if (deletes[pos]) {
				...
				ASSERT_TRUE((rc == RC_IM_KEY_NOT_FOUND),
						"entry was deleted, should not find it");
			} else {
				TEST_CHECK(findKey(tree, key, &rid));
				ASSERT_EQUALS_RID(insert[pos], rid,
						"did we find the correct RID?");
			}
		}
...
void testIndexScan(void) {
	...
	for (iter = 0; iter < 50; iter++) {
		...
		ASSERT_EQUALS_INT(RC_IM_NO_MORE_ENTRIES, rc,
				"no error returned by scan");
